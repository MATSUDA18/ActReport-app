...
/* アプリ全体の左右の余白を詰めて、横幅を最大限に活用する */
.block-container {
    padding-left: 0.8rem !important;
    padding-right: 0.8rem !important;
}

/* スマホでもカラムを絶対に横3列（各幅31%）に強制固定する */
...
/* サムネイル画像をさらに小さくスリムにし、3列できれいに収める */
div[data-testid="stColumn"] img,
.stColumn img {
    width: 60px !important;  /* 横幅をさらにコンパクトに調整 */
    height: 80px !important; /* 縦長比率を維持 */
            ...
            for i in range(0, len(uploaded_day_files), 3):
                ...
                for col_idx, uf in enumerate(row_files):
                    ...
                    with cols_add[col_idx]:
                        try:
                            st.image(Image.open(temp_path), width=60)
                        except Exception:
                            pass
...
with st.expander("⚙️ 【普段は閉じています】用意された画像の登録・削除管理"):
    ...
    if existing_images:
        ...
        st.info("登録されている画像はありません。上のフォームから画像を追加してください。")import streamlit as st
import datetime
import urllib.parse
import os
from PIL import Image
